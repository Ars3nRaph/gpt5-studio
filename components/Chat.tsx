...
function looksLikeCIExpired(msg: string){
  const s = msg.toLowerCase();
  return (s.includes('interpreter') && s.includes('expire')) || (s.includes('session') && s.includes('expire'));
}
...
export default function Chat(){
  ...
  const [ciExpired, setCiExpired] = useState(false);
  ...
  function updateActive(updater: (c: Conversation)=>Conversation){
    setConvos(list => list.map(c => c.id === active.id ? updater(c) : c));
  }
  function newChat(){
    const c: Conversation = { id: uid(), title: 'Nouvelle discussion', messages: [], files: [] };
    setConvos(l => [c, ...l]); setActiveId(c.id);
  }
  function renameActive(){
    const title = prompt('Nouveau titre ?', active.title); if (!title) return;
    updateActive(c => ({ ...c, title }));
  }
  function deleteActive(){
    if (!confirm('Supprimer cette conversation ?')) return;
    setConvos(list => {
      const filtered = list.filter(c => c.id !== active.id);
      const next = filtered[0] ?? { id: uid(), title: 'Nouvelle discussion', messages: [], files: [] };
      setActiveId(next.id);
      return filtered.length ? filtered : [next];
    });
  }
  ...
  const send = useCallback(async (overrideUser?: string, { forceCI }: { forceCI?: boolean } = {}) => {
    const text = (overrideUser ?? input).trim(); if (!text) return;
    setInput(''); setLastUser(text); setCiExpired(false); setLastError(null);
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        devMode,
        useCodeInterpreter: forceCI ?? useCI,
      })
    });
    if (!res.body) return;
    setStreaming(true);
      ...
      for (const e of events) {
        try {
          ...
          if (json.type === 'response.output_text.delta') {
            ...
            if (looksLikeCIExpired(json.error) && !autoRetried) {
              setCiExpired(true);
            }
          }
        } catch {}
  ...
  }, [input, active, devMode, useCI, autoRetried]);
  ...
  async function retryCI(){
    setAutoRetried(true);
    updateActive(c => ({ ...c, messages: c.messages.slice(0, -1) }));
    await send(lastUser || 'Relance de l’interpréteur', { forceCI: true });
    setCiExpired(false);
  }
  ...
  return (
    <div className="grid grid-cols-[280px_1fr] h-dvh">
      ...
      <main className="flex flex-col min-h-0">
        ...
        {ciExpired && (
          <div className="mx-4 mt-3 p-3 rounded-lg bg-amber-100 border border-amber-300 text-amber-900 text-sm flex items-center justify-between">
            <div>La session de l’interpréteur de code a expiré. Relancez pour continuer.</div>
            <div className="flex gap-2">
              <button onClick={retryCI} className="px-3 py-1 rounded-md bg-amber-900 text-white">Relancer l’interpréteur</button>
            </div>
          </div>
        )}
        {lastError && !ciExpired && (
          <div className="mx-4 mt-3 p-3 rounded-lg bg-red-100 border border-red-300 text-red-900 text-xs">
            Erreur: {lastError}
          </div>
        )}
